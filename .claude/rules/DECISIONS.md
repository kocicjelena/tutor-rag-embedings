...
| "Free on Hugging Face Spaces now, a ~€5/month VPS later" | **Open again — the free half no longer exists** | Hugging Face requires a paid plan (PRO, $9/month) to *create* a Gradio or Docker Space; only Static Spaces are free, and this app cannot be static. Discovered 2026-07-31, mid-deploy. The image is host-agnostic by design, so the options are the laptop (€0), a small VPS (~€4–5), or PRO ($9) — Jelena's call, in `DEPLOY-HF.md` |
